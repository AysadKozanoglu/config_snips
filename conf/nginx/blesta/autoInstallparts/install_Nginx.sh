...
wget -q http://s.tip90.com/source/packages/openssl-0.9.8zf.tar.gz
tar zxvf nginx-1.12.1.tar.gz
tar zxvf openssl-0.9.8zf.tar.gz
cd nginx-1.12.1 && ./configure --with-cc-opt='-g -O2 -fstack-protector-strong -Wformat -Werror=format-security -D_FORTIFY_SOURCE=2' --with-ld-opt=-Wl,-z,relro --sbin-path=/usr/local/sbin --with-http_stub_status_module --with-http_ssl_module --user=www-data --group=www-data --with-openssl=/source/openssl-0.9.8zf/ --with-stream make && make install
